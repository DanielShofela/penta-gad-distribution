...
import { TontineGroup, TontineMember, TontinePayment } from '../types';
import { 
  Users, 
  CreditCard, 
  Calendar, 
...
const TontineDashboard = () => {
  ...
  const [userTontines, setUserTontines] = useState<{group: TontineGroup, member: TontineMember}[]>([]);
  const [selectedTontine, setSelectedTontine] = useState<{group: TontineGroup, member: TontineMember} | null>(null);
  const [groupMembers, setGroupMembers] = useState<TontineMember[]>([]);
  const [payments, setPayments] = useState<TontinePayment[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'overview' | 'members' | 'timeline' | 'history'>('overview');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snap) => {
      ...
      for (const mDoc of snap.docs) {
        ...
        try {
          const gDoc = await getDoc(doc(db, 'tontine_groups', member.groupId));
          if (gDoc.exists()) {
  ...
  useEffect(() => {
    ...
    const unsubPayments = onSnapshot(paymentsQ, (snap) => {
      setPayments(snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as TontinePayment)));
    }, (error) => handleFirestoreError(error, OperationType.LIST, 'tontine_payments'));
  ...
  if (userTontines.length === 0) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2rem] p-12 border border-gray-100 shadow-sm">
          <div className="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-8 text-blue-900 shadow-inner">
            <Users size={48} />
          </div>
          <h2 className="text-3xl font-black text-blue-900 mb-4 italic tracking-tight">Aucune tontine active</h2>
          <p className="text-gray-500 mb-8 text-lg leading-relaxed max-w-md mx-auto">
            Vous n'avez pas encore rejoint de groupe de tontine. Choisissez un article éligible et commencez à cotiser avec la communauté.
          </p>
          <motion.button 
  ...
  const progressPercent = (member.totalPaid / (member.totalPaid + member.remainingAmount)) * 100;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] mt-2 ml-16">Système de paiement collaboratif intelligent</p>
        </div>

        <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-gray-100 shadow-sm group">
          <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-4">Mes Groupes</span>
             ...
             {userTontines.map(t => (
               <button 
                 ...
                 className={cn(
                   ...
                   selectedTontine?.group.id === t.group.id ? "bg-blue-900 text-white shadow-lg" : "bg-gray-50 text-gray-400 hover:bg-sky-50"
                 )}
               >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
           ...
           >
              <div className="absolute top-0 right-0 p-4">
                 <div className="bg-yellow-400 text-blue-900 text-[10px] font-black uppercase px-3 py-1 rounded-full shadow-md">
                    {group.status === 'waiting' ? "En attente" : group.status === 'active' ? "En cours" : "Terminé"}
                 </div>
              </div>
              <img src={group.productImage} className="w-full aspect-square object-contain mb-6 group-hover:scale-110 transition-transform duration-700" alt="" />
              <h3 className="text-xl font-black text-blue-900 mb-2 leading-tight uppercase italic">{group.productName}</h3>
              <div className="flex items-center justify-between mt-4 border-t border-gray-50 pt-4">
                 <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Valeur</p>
                    <p className="font-black text-blue-900">{formatCurrency(member.totalPaid + member.remainingAmount)}</p>
                 </div>
                 <div className="text-right">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Membres</p>
                    <p className="font-black text-blue-900 flex items-center justify-end gap-1">
                       {group.currentMembers} <span className="text-gray-300">/ {group.totalMembers}</span>
                    </p>
                 </div>
              </div>
           </motion.div>
           ...
           <div className="bg-blue-900 rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                 <div className="flex items-center gap-3 mb-6">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-60">Ma Position</span>
                 </div>
                 <div className="space-y-6">
                    <div>
                       <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Rang</p>
                       <p className="text-4xl font-black italic tracking-tighter">#{member.rank}</p>
                    </div>
        ...
        <div className="lg:col-span-3 space-y-8">
           ...
           <div className="flex items-center gap-8 border-b border-gray-100 pb-0">
              {[
                { id: 'overview', label: 'Surroundings', icon: LayoutDashboard },
                ...
                <button 
                  ...
                  className={cn(
                    ...
                    activeTab === tab.id ? "border-blue-900 text-blue-900" : "border-transparent text-gray-400 hover:text-blue-500"
                  )}
                >
                  <tab.icon size={16} />
                  {tab.label}
           ...
           <AnimatePresence mode="wait">
                   ...
                   <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm flex flex-col justify-between">
                      <div>
                         <h4 className="text-xs font-black text-blue-900 uppercase tracking-widest mb-6 flex items-center gap-2">
                            <Clock size={16} className="text-yellow-500" /> Prochain Événement
                         </h4>
                         <div className="space-y-4">
                            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl">
                               <div className="w-12 h-12 bg-white rounded-xl shadow-sm flex flex-col items-center justify-center font-black">
                                  <span className="text-[10px] text-blue-900 opacity-40 leading-none">{format(member.nextPaymentDate?.toDate() || new Date(), 'MMM', { locale: fr })}</span>
                                  <span className="text-xl text-blue-900 leading-none">{format(member.nextPaymentDate?.toDate() || new Date(), 'dd')}</span>
                               </div>
                               <div>
                                  <p className="text-xs font-black text-blue-900 uppercase">Prochaine Cotisation</p>
                                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">À valider avant 18h00</p>
                               </div>
                            </div>
                            <div className="flex items-center gap-4 p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50">
                               ...
                               <div>
                                  <p className="text-xs font-black text-blue-900 uppercase">Réception Estimée</p>
                                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">Après le cycle #{member.rank}</p>
                               </div>
                            </div>
                         </div>
                      </div>
                      <div className="mt-8 pt-6 border-t border-gray-50">
                         <div className="flex items-center justify-between text-xs">
                             <span className="text-gray-400 font-bold uppercase tracking-widest">Total Payé</span>
                             <span className="font-black text-blue-900">{formatCurrency(member.totalPaid)}</span>
                         </div>
                         <div className="flex items-center justify-between text-xs mt-2">
                             <span className="text-gray-400 font-bold uppercase tracking-widest">Reste</span>
                             <span className="font-black text-red-500">{formatCurrency(member.remainingAmount)}</span>
                         </div>
                      </div>
                   </div>
                      ...
                      <div className="space-y-6">
                         <div className="relative">
                            <svg className="w-32 h-32 mx-auto rotate-[-90deg]">
                               ...
                               <motion.circle 
                                 ...
                                 animate={{ strokeDashoffset: 364 - (364 * (group.currentMembers / group.totalMembers)) }}
                                 className="text-blue-900"
                               />
                            </svg>
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                               <span className="text-2xl font-black italic text-blue-900 leading-none">{group.currentMembers}</span>
                               <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Membres</span>
                            </div>
                         </div>
                         ...
                         <div className="grid grid-cols-2 gap-4 mt-6">
                            <div className="text-center p-4 bg-gray-50 rounded-2xl">
                               ...
                               <p className="text-xs font-black text-blue-900 uppercase tracking-tight">{group.contributionFrequency === 'daily' ? 'Journalière' : 'Hebdo'}</p>
                            </div>
                            <div className="text-center p-4 bg-gray-50 rounded-2xl">
                               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Mise</p>
                               <p className="text-xs font-black text-blue-900 uppercase tracking-tight">{formatCurrency(group.contributionAmount)}</p>
                            </div>
                         </div>
                      </div>
                   </div>
                ...
                >
                   <div className="flex items-center justify-between mb-8">
                      <h4 className="text-xs font-black text-blue-900 uppercase tracking-widest flex items-center gap-2">
                         <Users size={16} className="text-yellow-500" /> Membres du Groupe
                      </h4>
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">{group.currentMembers} actifs</span>
                   </div>
                        ...
                        <div key={m.id} className={cn(
                          ...
                          m.userId === user?.uid ? "bg-blue-50 border-blue-100 shadow-sm scale-105" : "bg-white border-gray-100 hover:border-blue-200"
                           ...
                           <div className="flex-1 overflow-hidden">
                              <p className="text-xs font-black text-blue-900 truncate uppercase mt-1">{m.userName} {m.userId === user?.uid && "(Moi)"}</p>
              ...
              {activeTab === 'timeline' && (
                ...
                >
                   <h4 className="text-xs font-black text-blue-900 uppercase tracking-widest mb-12 flex items-center gap-2">
                       <Calendar size={16} className="text-yellow-500" /> Séquence de Remise
                   </h4>
                   <div className="relative pl-12 space-y-12 before:content-[''] before:absolute before:left-[1.85rem] before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100">
                      {[...Array(group.totalMembers)].map((_, i) => {
                        const rank = i + 1;
                        const m = groupMembers.find(member => member.rank === rank);
                        ...
                        return (
                          <div key={rank} className="relative group/step">
                             <div className={cn(
                               ...
                               isPast ? "bg-green-500 scale-90" : 
                               isCurrent ? "bg-yellow-400 scale-125 shadow-yellow-400/50" : 
                               "bg-gray-200"
                             )}>
                                {isPast && <CheckCircle size={10} className="text-white mx-auto mt-0.5" />}
                             </div>
                             <div className={cn(
                               "p-6 rounded-3xl border transition-all duration-500 flex items-center justify-between",
                               isCurrent ? "bg-blue-900 text-white border-blue-900 shadow-2xl translate-x-2" : 
                               isUser ? "bg-blue-50 border-blue-100" :
                                   ...
                                   <div>
                                      ...
                                      <p className={cn(
                                        ...
                                        isCurrent ? "text-blue-200" : "text-gray-400"
                                         ...
                                         {isCurrent ? "En possession de l'article" : isPast ? "Livré avec succès" : "A venir"}
                                      </p>
                                   </div>
                                </div>
              ...
              {activeTab === 'history' && (
                ...
                >
                   <div className="flex items-center justify-between mb-8">
                      <h4 className="text-xs font-black text-blue-900 uppercase tracking-widest flex items-center gap-2">
                         <History size={16} className="text-yellow-500" /> Journal des Cotisations
                      </h4>
                      <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-xl border border-gray-100">
                         <Search size={14} className="text-gray-400" />
                         <input type="text" placeholder="Rechercher..." className="bg-transparent border-none outline-none text-[10px] font-bold w-32" />
                      </div>
                   </div>
                   
                   <div className="space-y-3">
                      {payments.map(p => (
                        <div key={p.id} className="group p-4 bg-gray-50/50 hover:bg-white border hover:border-blue-100 rounded-2xl flex items-center justify-between transition-all">
                           <div className="flex items-center gap-4">
                              <div className="p-3 bg-white rounded-xl shadow-sm group-hover:scale-110 transition-transform">
                                 <Zap size={16} className="text-blue-900" />
                              </div>
                              <div>
                                 <p className="text-xs font-black text-blue-900 uppercase">{p.paymentMethod}</p>
                                 <p className="text-[10px] text-gray-400 font-bold font-mono tracking-tighter">REF: {p.transactionId}</p>
                              </div>
                           </div>
                           <div className="text-right">
                              <p className="text-sm font-black text-blue-900">-{formatCurrency(p.amount)}</p>
                              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{format(p.paymentDate.toDate(), 'dd MMM HH:mm', { locale: fr })}</p>
                           </div>
                        </div>
                      ))}
                      {payments.length === 0 && (
                        <div className="py-12 text-center">
                           ...
                           <p className="text-xs font-bold text-gray-400 uppercase tracking-[0.2em] italic">Aucun mouvement récent</p>
                        </div>
                      )}
                   </div>
                </motion.div>
